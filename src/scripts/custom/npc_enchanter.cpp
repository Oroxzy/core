...
{
    return slot >= EQUIPMENT_SLOT_START && slot < EQUIPMENT_SLOT_END;
}

static Item* GetEquippedItemForSlot(Player* player, uint8 slot)
{
    if (!player || !IsEnchantableEquipmentSlot(slot))
    {
        return NULL;
    }
...
static uint32 GetCurrentEnchantId(Player* player, uint8 slot)
{
    Item* pItem = GetEquippedItemForSlot(player, slot);
    if (!pItem)
    {
        return 0;
    }

    uint32 enchantId = pItem->GetEnchantmentId(PERM_ENCHANTMENT_SLOT);
    if (!enchantId)
    {
        return 0;
    }
...
static bool IsItemCompatibleForEnchant(Item* pItem, SpellEntry const* spellInfo)
{
    ...
    if (spellInfo->EquippedItemClass != -1 && int32(pProto->Class) != spellInfo->EquippedItemClass)
    {
        return false;
    }
    ...
    {
        if (pProto->SubClass >= 32 || !((1u << pProto->SubClass) & spellInfo->EquippedItemSubClassMask))
        {
            return false;
        }
    }
    ...
    {
        if (pProto->InventoryType >= 32 || !((1u << pProto->InventoryType) & spellInfo->EquippedItemInventoryTypeMask))
        {
            return false;
        }
    }
...
void EnchantItem(Player* player, GameObject* /*gameobject*/, uint32 spellid, uint8 slot)
{
    ...
    if (!pItem)
    {
        player->GetSession()->SendNotification("Your item could not be enchanted, there is no item equipped in the specified slot.");
        return;
    }
    ...
    if (!pProto)
    {
        player->GetSession()->SendNotification("Your item data could not be loaded.");
        return;
    }
    ...
    if (!enchantid)
    {
        player->GetSession()->SendNotification("Invalid enchant effect on spell.");
        return;
    }

    if (!IsItemCompatibleForEnchant(pItem, spellInfo))
    {
...
void RemoveEnchantItem(Player* player, GameObject* /*gameobject*/)
{
    ...
    bool removedAnyEnchant = false;

    for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
    {
        Item* equippedItem = GetEquippedItemForSlot(player, i);

        if (equippedItem && equippedItem->GetEnchantmentId(PERM_ENCHANTMENT_SLOT))
        {
            ...
            removedAnyEnchant = true;
        }
    }

    if (removedAnyEnchant)
    {
        player->CastSpell(player, COOL_VISUAL_SPELL_1, true);
        player->CastSpell(player, COOL_VISUAL_SPELL_2, true);
        player->GetSession()->SendAreaTriggerMessage("All permanent enchants were removed.");
    }
    else
    {
        player->GetSession()->SendNotification("You have no permanent enchants to remove.");
    }
}
...
bool GossipSelect_EnchanterNPC(Player* player, GameObject* gameobject, uint32 uiSender, uint32 uiAction)
{
    ...
    {
        if (!IsEnchantableEquipmentSlot(uint8(uiSender)))
        {
            player->GetSession()->SendNotification("Invalid equipment slot.");
            GossipHello_EnchanterNPC(player, gameobject);
            return true;
        }

        EnchantItem(player, gameobject, uiAction, uint8(uiSender));
        //player->CLOSE_GOSSIP_MENU();
        GossipHello_EnchanterNPC(player, gameobject);
    }
    return true;
