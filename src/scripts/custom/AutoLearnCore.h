...
static uint32 LevelToNextTenAndLearn(Player* pPlayer, Creature* pCreatureCaster)
{
    ...
    // ------------------------------------------------------------
    // Public Wrapper API fuer andere Scripts (Template NPC usw.)
    // ------------------------------------------------------------
    static inline uint32 LearnAllAtCurrentLevel(Player* pPlayer, Creature* pCreatureCaster)
    {
        return LearnCurrentLevel(pPlayer, pCreatureCaster);
    }

    static inline uint32 LevelPlusTenAndLearnAll(Player* pPlayer, Creature* pCreatureCaster)
    {
        return LevelPlusTenAndLearn(pPlayer, pCreatureCaster);
    }

    static inline uint32 LevelToNextTenAndLearnAll(Player* pPlayer, Creature* pCreatureCaster)
    {
        return LevelToNextTenAndLearn(pPlayer, pCreatureCaster);
    }

    static inline uint32 LevelToAndLearnAll(Player* pPlayer, Creature* pCreatureCaster, uint32 targetLevel)
    {
        return LevelToAndLearn(pPlayer, pCreatureCaster, targetLevel);
    }
