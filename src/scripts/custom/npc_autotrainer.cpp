...
struct AutoTrainerSource
{
    uint32 trainerId;
    uint32 trainerEntry;
    bool   isPetTrainer; // true only for pet trainers (Hunter)
...
static uint32 FindNextSpellInChainLinear(uint32 spellId)
{
    // Purpose: Find the next spell in a chain without relying on SpellMgr::GetNextSpellInChain.
    // Some vMaNGOS branches only expose GetPrevSpellInChain.
    if (!spellId)
        return 0;
...
static uint32 ResolveTalentSpellIdForRank(uint32 talentSpellId, uint8 rank)
{
    // Purpose: Convert a base talent spell id plus rank into the concrete spell id.
    if (!talentSpellId)
        return 0;
...
{
    // Purpose: Learn template talent spells using the stored spell id plus rank.
    if (!pPlayer)
        return;
...
{
    // Purpose: Save the current hunter pet as a selectable pet template.
    if (!pPlayer || pPlayer->GetByteValue(UNIT_FIELD_BYTES_0, 1) != CLASS_HUNTER)
        return;
    ...
    std::string escapedName = cInfo->name;
    CharacterDatabase.escape_string(escapedName);

    CharacterDatabase.PExecute(
        "INSERT INTO template_pets (name, entry, PetFamily, AttackSpeed) VALUES ('%s', '%u', '%u', '%u')",
        escapedName.c_str(),
        pet->GetEntry(),
        cInfo->pet_family,
        cInfo->base_attack_time);
}
...
{
    // Purpose: ASCII case-insensitive contains
    if (!needle || !*needle)
        return true;
...
{
    // Purpose: Wrapper for const char*.
    if (!needle || !*needle)
        return true;
...
{
    // Purpose: Max a skill only if the player already knows it.
    if (!pPlayer || !skillId)
        return;
...
{
    // Purpose: Max weapon, defense, and riding skills to the current core cap.
    if (!pPlayer)
        return;

    uint16 maxV = pPlayer->GetSkillMaxForLevel();
...
static void ResolveTrainerSourcesForClass(uint8 playerClass)
{
    ...
    for (CreatureInfoMap::const_iterator it = m.begin(); it != m.end(); ++it)
    {
        ...
        // Demon trainers are intentionally excluded here because warlock grimoires are hard-coded.
        if (isDemonTrainer)
            continue;

        // Normal case: only regular class trainers.
        // Exception: hunter pet trainers are allowed as well.
        if (!isPetTrainer)
        {
        ...
        switch (playerClass)
        {
            ...
            case CLASS_WARLOCK:
                ok = StrContainsI(cInfo->subname, "Warlock Trainer"); // Demon trainers are already excluded above.
                break;
...
{
    // Purpose: Collect weapon master sources once.
...
{
    // Purpose: Fetch TrainerSpellData, preferring trainer entry over trainer template.
    if (src.trainerEntry)
    {
...
{
    // Purpose: Build the next-in-chain cache once.
...
{
    // Purpose: Learn higher ranks from spell chains.
    if (!pPlayer)
        return 0;
...
{
    // Purpose: Return the minimum level for special or quest-related spells.
    switch (spellId)
    {
        case 688:   return 1;
        case 697:   return 10;
...
{
    // Purpose: Learn direct special spells such as quest rewards.
    if (!pPlayer || !spellId)
        return false;
...
{
    // Purpose: Learn a special spell only if class and race match.
    if (!pPlayer || !spellId)
        return false;
...
{
    // Purpose: Whitelist quest and special spells per class.
    if (!pPlayer)
        return 0;
...
{
    // Purpose: Execute a trainer spell via a triggered cast on the target, just like a trainer.
    if (!pPlayer || !trainerSpell || !target)
        return false;

    // Crash safety: the target must still be valid in the world.
    if (!target->IsInWorld())
        return false;
...
{
    // Purpose: Run one pass over TrainerSpellData and teach only GREEN spells.
...
{
    // Purpose: Cast a teach or item spell (grimoire) on the player as a triggered cast.
    if (!pPlayer || !spellId)
        return false;
...
static uint32 LearnAllAvailableInLoop(Player* pPlayer, Creature* pCreatureCaster)
{
    // Purpose: Run multiple passes until nothing new can be learned.
    if (!pPlayer)
        return 0;
    ...
    // Learn warlock grimoires only once per full training pass to avoid repeated summons.
...
{
    // Purpose: Set the level and then learn everything available.
    if (!pPlayer)
        return 0;
...
bool GossipSelect_npc_autotrainer(Player* pPlayer, Creature* pCreature, uint32 sender, uint32 action)
{
    ...
    if (learned == 0)
        pPlayer->GetSession()->SendNotification("There is nothing new to learn right now.");
    else
        pPlayer->GetSession()->SendNotification("Learned: %u spells/abilities.", learned);
